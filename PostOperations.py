...
def Post(products,Operation):
    for p in products:
        ...
        if(Operation & bitwiseProductDesc):
        ...
        if(Operation & bitwiseProductUnit):
            ...
            productUnitId = int(retJSON['id'])

        if(Operation & bitwiseProductGtin):
            if 'E1WBB04' in E1WBB03:
                gtin                = E1WBB04['EAN11']
                gtinType            = 'NONE'
                mainGtinFlag        = True
                productGTIN         = ProductGTIN(gtin, gtinType, mainGtinFlag,productUnitId)
        ...
        if(Operation & bitwisePlanogram):
            planogram           = Planogram(numberOfFacings,orientationYaw,positionX,productUnitId,shelfLayerId,versionTimestamp)
